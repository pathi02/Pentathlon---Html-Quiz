...
export default function AdminDashboard() {
  ...
  const handleRemoveSubmission = async (token) => {
    if (!confirm("Are you sure you want to remove this submission?")) return;

    try {
      const adminToken = localStorage.getItem("adminToken");
      const response = await fetch(`/api/admin/submissions/${token}`, {
        method: "DELETE",
                ...
                {submissionsLoading ? (
                  ...
                  submissions.map((submission, index) => {
                    const maxScore = submission.breakdown
                      ? Object.values(submission.breakdown).reduce(
                          (sum, b) => sum + (b.possible || 0),
                          0,
                        )
                      : 100;
                    ...
                    return (
                      <tr key={submission.token}>
                        ...
                        <td>
                          <button
                            onClick={() =>
                              handleRemoveSubmission(submission.token)
                            }
              ...
              <div className={styles.quizEditor}>
                <div className={styles.formGroup}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setQuiz({
                        ...quiz,
                        durationSeconds: parseInt(e.target.value) || 1800,
                      })
                    }
                ...
                <div className={styles.formGroup}>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setQuiz({ ...quiz, htmlTemplate: e.target.value })
                    }
                        ...
                        <div className={styles.blankRow}>
                          <div className={styles.formGroup}>
                            ...
                            <input
                              ...
                              value={blank.id}
                              onChange={(e) =>
                                updateBlank(index, "id", e.target.value)
                              }
                              className={styles.input}
                            />
                          </div>
                          <div className={styles.formGroup}>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateBlank(index, "expected", e.target.value)
                              }
                              className={styles.input}
                            />
                          </div>
                        ...
                        <div className={styles.blankOptions}>
                          <label>
                            <input
                              ...
                              onChange={(e) =>
                                updateBlank(index, "trim", e.target.checked)
                              }
                            />
                            Trim whitespace
                          </label>
